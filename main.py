...
from src.utils import check_csv, update_csv


def parse_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--is_historical", "--ih", action="store_true")
    parser.add_argument("--day_start", "--ds", type=int, required=True, help="yyyymmdd")
    parser.add_argument("--day_end", "--de", type=int, required=True, help="yyyymmdd; end date excluded from optimization")
    ...
    parser.add_argument("--rest_before_charge", "--rbc", type=float, required=False, default=0, help="rest before charge; h; int")
    parser.add_argument("--rest_after_discharge", "--rad", type=float, required=False, default=0, help="rest after discharge; h; int")
...
def validate_args(args):
    if args.day_start >= args.day_end:
        raise ValueError()
    if (args.is_historical) and ((args.day_start < 20150101) or (args.day_end > 20250101)):
        raise ValueError()
    if (args.power_output_rated <= 0) or (args.energy_capacity_rated <= 0) or (args.energy_capacity_rated < args.power_output_rated):
        raise ValueError()
...
def main():
    ...
    time_start = get_datetime64(args.day_start, False)
    time_end = get_datetime64(args.day_end, True)
    ...
    if not args.return_details:
        update_csv(output, args)
    else:
        pass
