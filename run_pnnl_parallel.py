...
def get_smp_monthly(args):
    ...
    global timestamp_monthly_str, num_months, num_years, smp_monthly, smp_ref_monthly
    timestamp_monthly_str = timestamp_monthly.astype(str).tolist()
    num_months = len(timestamp_monthly_str)
...
def per_csv(path_inputs_csv_file, path_outputs_csv_file_profit, path_outputs_csv_file_revenue):
    ...
    pd.DataFrame(
        arbitrage_profit.reshape(num_configs, num_years, 12).sum(axis=2),
        columns=timestamp_monthly_str,
    ).to_csv(path_outputs_csv_file_profit, index=False)
    pd.DataFrame(
        discharging_revenue.reshape(num_configs, num_years, 12).sum(axis=2),
        columns=timestamp_monthly_str,
